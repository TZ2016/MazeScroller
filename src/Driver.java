...
public class Driver extends BasicGame {	
	...
	private static void setUp() {
		...
		int dif = 0, w = 0, h = 0, dim = 0, intv = 0, jbon = 0;
		while(!legalargs) {
			Util.print("Choose density, max width, max height, dimension, interval\n");
			try {
				dif = sc.nextInt();
				w = sc.nextInt();
				h = sc.nextInt();
		...
		Maze.mazeDensity = dif*0.1;
		
		// default value
		Scene.DIMENSION = dim; // depends on user input
		interval = intv;
